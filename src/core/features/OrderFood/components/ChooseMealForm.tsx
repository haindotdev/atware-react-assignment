...
interface Props {
  onNext: (data: Partial<{ meal: string; people: number }>) => void;
}

export default function ChooseMealForm({onNext}: Readonly<Props>) {
  ...
  return <form onSubmit={form.handleSubmit(onSubmit)}>
    <div>
      ...
      <div className='w-full max-w-sm'>
        ...
        <input
          type='number'
          value={form.getValues('people')}
          onChange={(e) => form.setValue('people', parseInt(e.target.value) || 1)}
          min={1}
          max={10}
          className='w-full p-3 border-2 border-black rounded-md font-bold focus:outline-none'
        />
